<template src="./driver-profile.html"></template>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.vehicles_preview = this.user.vehicle_photos;
        },
        mounted(){
            this.loadTransports();
        },
        methods: {
            ...
            vehicleUploaded(event) {
                this.vehicles_preview.splice(0, this.vehicles_preview.length);
                let files = [...event.target.files];
                files.forEach(file => {
                    this.vehicle.pictures.push(file);
                    this.previewVehicle(file);
                });
            },
            previewVehicle(file){
                let photo = {};
                ...
                reader.readAsDataURL(file);
                reader.onloadend = () => {
                    photo.photoUrl = reader.result;
                    this.vehicles_preview.push(photo);
                }
            },
            openProfileModal(){
